...
Typical use:
    python P2_sfincs_couple_adcirc_cama_tc_rain_future_batch.py
    python P2_sfincs_couple_adcirc_cama_tc_rain_future_batch.py --site PRD
    python P2_sfincs_couple_adcirc_cama_tc_rain_future_batch.py --site PRD --scenarios ssp585_mean --return-periods rp100yr
    python P2_sfincs_couple_adcirc_cama_tc_rain_future_batch.py --all-sites
...
REPOSITORY_ROOT = EXAMPLES_ROOT.parent

FUTURE_ADCIRC_ROOT = Path(
    os.environ.get(
        "FUTURE_ADCIRC_ROOT",
        REPOSITORY_ROOT / "external" / "adcirc" / "future_adirc_fort",
    )
)
...
FLOW_STATS_ROOT = Path(
    os.environ.get("CAMA_FLOW_STATS_ROOT", REPOSITORY_ROOT / "external" / "camaflood" / "result")
)
...
CAMA_RUN_SCRIPT_TEMPLATE = (
    os.environ.get(
        "CAMA_RUN_SCRIPT_TEMPLATE",
        str(REPOSITORY_ROOT / "external" / "camaflood" / "gosh" / "auto_cama_vic_coupled_module_{region_tag}.sh"),
    )
)
CAMA_OUTFLOW_BIN_TEMPLATE = (
    os.environ.get(
        "CAMA_OUTFLOW_BIN_TEMPLATE",
        str(REPOSITORY_ROOT / "external" / "camaflood" / "gosh" / "work_{region_name}_vic_camaflood" / "camaout" / "outflw{year}.bin"),
    )
)

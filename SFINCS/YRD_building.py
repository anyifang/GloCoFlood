...
MODEL_FOLDER = "YRD_SFINCS"  

MODEL_BBOX = [119.2, 29.5, 123.0, 32.5]
...
RIVER_SLOPE = 1e-5  
MAX_RIVER_UPSTREAM_M = 180000  
BOX_EDGE_CHECK_CELLS = 15
CAMA_SFINCS_MATCH_MAX_M = 80000
CAMA_BOX_EDGE_PRIORITY_M = 80000
...
# 核心修正：分离淘汰阈值与渲染阈值
SEA_DIKE_HEIGHT_M = 2.0
RIVER_DIKE_INLAND_HEIGHT_M = 8.0
RIVER_DIKE_GRADIENT_DISTANCE_M = 80000.0
...
def apply_terrain_dikes_after_terrain(
    ...
    inflow_mask = np.zeros_like(final_river_mask, dtype=bool)
    if inflow_points:
        inv_transform = ~sf.grid.raster.transform
        nrows, ncols = inflow_mask.shape
        for src_pt in inflow_points:
            if src_pt is None or src_pt.is_empty:
                continue
            col_f, row_f = inv_transform * (float(src_pt.x), float(src_pt.y))
            ix_src = int(np.floor(col_f))
            iy_src = int(np.floor(row_f))
            if 0 <= iy_src < nrows and 0 <= ix_src < ncols:
                inflow_mask[iy_src, ix_src] = True

    if np.any(inflow_mask):
        dist_to_inflow_m = distance_transform_edt(
            ~inflow_mask,
            sampling=(pixel_size_m, pixel_size_m),
        )
    else:
        dist_to_inflow_m = np.full_like(current_dep, np.inf, dtype=float)
    ...
    river_dike_mask = (
        ...
        & land_mask
        & np.isfinite(current_dep)
        & (dist_to_inflow_m <= gradient_distance_m)
    )
    ...
    river_dike_raise = (
        float(river_dike_inland_height_m)
        + river_gradient_factor
        * (float(sea_dike_height_m) - float(river_dike_inland_height_m))
    )
    ...
    print(
        ...
        f"river inflow=+{float(river_dike_inland_height_m):.2f} m "
        f"decays to +{float(sea_dike_height_m):.2f} m over "
        f"{float(river_dike_gradient_distance_m) / 1000.0:.1f} km from src"
    )
    if np.any(river_dike_mask):
        ...
        print(
            "   [WARNING] No river dike cells were raised near inflow boundaries. "
            f"inflow_points={0 if inflow_points is None else len(inflow_points)}"
        )
...
apply_terrain_dikes_after_terrain(
    ...
    inflow_points=valid_src_points,
)

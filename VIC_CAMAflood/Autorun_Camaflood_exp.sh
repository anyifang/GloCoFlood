...
if [[ -n "${CAMA_DIR:-}" && -d "${CAMA_DIR}" ]]; then
  BASE="${CAMA_DIR}"
else
  BASE="${SCRIPT_DIR}/../external/camaflood"
fi
...
if [[ -n "${CAMA_RUNTIME_MODULE:-}" ]] && command -v module >/dev/null 2>&1; then
  module load "$CAMA_RUNTIME_MODULE"
fi
export IFORTLIB="${IFORTLIB:-}"
export HDF5LIB="${HDF5LIB:-}"
...
EXP="PRD_test"                       # experiment name or absolute case directory
if [[ "${EXP}" = /* ]]; then
  EXP_ROOT="${EXP}"
else
  EXP_ROOT="${BASE}/gosh/${EXP}"
fi
RDIR="${EXP_ROOT}/camaout"                  # directory to run CaMa-Flood
...
FMAP="${BASE}/map/reg_PRD_Auto_15min"                # map directory
...
CDIMINFO="${FMAP}/diminfo_vic_15min.txt"
CINPMAT="${FMAP}/inpmat_vic_15min.bin"

IFRQ_INP="6"                               # input forcing frequency: [1,2,3,...,24] hour
DROFUNIT="3600000"   # [mm/day->m/s]       # runoff unit conversion
...
CROFDIR="${EXP_ROOT}/cama_runoff_from_vic"          # runoff directory
CROFPRE="vic_runoff_"   # runoff prefix/suffix  

...
# 模块环境
MODULE_INIT_SH="${MODULE_INIT_SH:-}"
PYTHON_MODULE="${PYTHON_MODULE:-miniforge/25.3}"
PYTHON_BIN="${PYTHON_BIN:-python}"
VIC_RUNTIME_MODULE="${VIC_RUNTIME_MODULE:-netcdf/4.4.1-icc-kd}"

# 基础编译/运行库
if [[ -n "$MODULE_INIT_SH" ]]; then source "$MODULE_INIT_SH"; fi
if command -v module >/dev/null 2>&1; then
  module load "${VIC_RUNTIME_MODULE}" || true
  [[ -z "${MPI_RUNTIME_MODULE:-}" ]] || module load "${MPI_RUNTIME_MODULE}" || true
fi
...
# 根目录与区域
CAMA_DIR="${CAMA_DIR:?Set CAMA_DIR to the CaMa-Flood package root}"
HYDROMT_SFINCS_EXAMPLES="${HYDROMT_SFINCS_EXAMPLES:?Set HYDROMT_SFINCS_EXAMPLES}"
GEOJSON_PATH="${GEOJSON_PATH:-${HYDROMT_SFINCS_EXAMPLES}/Misp_SFINCS/cama_upstream_rivers.geojson}"
REGION_NAME="Misp_Auto"
...
VIC_EXEC="${VIC_EXEC:?Set VIC_EXEC to the VIC Image Driver executable}"

# ERA5 -> VIC forcing
VIC_FORCING_MODE="ERA5_DAILY_TO_VIC"   # READY_VIC_NC or ERA5_DAILY_TO_VIC; 调试阶段建议切到 READY_VIC_NC 复用已生成 forcing
VIC_READY_FORCING_PREFIX="${VIC_READY_FORCING_PREFIX:-${CAMA_DIR}/VIC_Data/vic_forcing/vic_forcing_}"
ERA5_DAILY_ROOT="${ERA5_DAILY_ROOT:-${CAMA_DIR}/VIC_Data/ERA5_global}"
...
# 参数数据源
DIR_OPENLANDMAP="${VIC_DATA_ROOT:-${CAMA_DIR}/VIC_Data}"
LAND_DEM_NC="${LAND_DEM_NC:-${HYDROMT_SFINCS_EXAMPLES}/data/gebco/GEBCO_2025_sub_ice.nc}"

...
# 1. 用户配置区
# =========================================================
CAMA_DIR = os.environ.get("CAMA_DIR", "../../cmf_v420_pkg")
GEOJSON_PATH = os.environ.get(
    "SFINCS_CAMA_RIVER_GEOJSON",
    "../../../HydroMT-SFINCS/examples/PRD_SFINCS/cama_upstream_rivers.geojson",
)
...
# ---------- VIC Image Driver ----------
VIC_EXEC = os.environ.get("VIC_EXEC", "vic_image.exe")
RUN_VIC = True
...
VIC_READY_FORCING_PREFIX = os.environ.get(
    "VIC_READY_FORCING_PREFIX", "VIC_Data/vic_forcing/vic_forcing_"
)

# 你当前 ERA5 下载结果的根目录（按年分子目录）
ERA5_DAILY_ROOT = os.environ.get("ERA5_DAILY_ROOT", os.path.join(CAMA_DIR, "VIC_Data", "ERA5"))
...
ERA5_INPUT_TEMPLATE = os.environ.get(
    "ERA5_INPUT_TEMPLATE", os.path.join("VIC_Data", "ERA5", "era5_{year}.nc")
)
...
# ---------- VIC 参数数据 ----------
DIR_OPENLANDMAP = os.environ.get("VIC_DATA_ROOT", os.path.join(CAMA_DIR, "VIC_Data"))
LAND_DEM_NC = os.environ.get("LAND_DEM_NC", os.path.join("data", "gebco", "GEBCO_2025_sub_ice.nc"))
GLOBAL_SOIL_TXT = os.environ.get(
    "GLOBAL_SOIL_TXT", os.path.join(DIR_OPENLANDMAP, "global", "global_soil_param_new.txt")
)
ANNUAL_PRECIP_TIF = os.environ.get(
    "ANNUAL_PRECIP_TIF", os.path.join(ERA5_DAILY_ROOT, "annual_mean_precip.tif")
)
